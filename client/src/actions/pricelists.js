...
export const getPriceList = (token) => async (dispatch) => {
  try {
    await fetch(`${REACT_APP_URL_API}/pricelist`, {
      headers: {
...
export const getPriceListsById = (token, id) => async (dispatch) => {
  try {
    fetch(`${REACT_APP_URL_API}/pricelist/${id}`, {
      headers: {
        "Content-Type": "application/json",
        "auth-token": token,
      },
...
export const addPriceList =
  (data, token, enqueueSnackbar, closeSnackbar) => async (dispatch) => {
    try {
      await fetch(`${REACT_APP_URL_API}/pricelist/createpricelist`, {
        method: "POST",
        ...
        .then((res) => {
          if (
            res.status === 400 &&
            res.message === "Price list already exists"
          ) {
...
export const editPriceList =
  (id, data, token, enqueueSnackbar, closeSnackbar) => async (dispatch) => {
    try {
      await fetch(`${REACT_APP_URL_API}/pricelist/editpricelist/${id}`, {
        method: "POST",
