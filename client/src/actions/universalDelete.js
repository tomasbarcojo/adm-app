...
export const universalDelete =
  (path, id, token, enqueueSnackbar, closeSnackbar) => async (dispatch) => {
    try {
      await fetch(`${REACT_APP_URL_API}/${path}/${id}`, {
        method: "DELETE",
        headers: {
        ...
        .then((res) => {
          if (res.status === 200) {
            ...
            enqueueSnackbar(
              "El sistema intento borrar con un ID no valido. (Esto no deberia pasar)",
              {
                variant: "error",
                action: (key) => (
                  <button
