...
export default function CustomTable() {
  ...
  useEffect(() => {
    //to clear if there are data on pricelistsbyid and show correct information
        ...
        <CardHeader color="primary">
          <div className={classes.card}>
            <h4 className={classes.cardTitleWhite}>
              Editar listado: {pricelists[0] && pricelists[0].pricelist.priceListName}
            </h4>
            <Button color="info" onClick={handleEditPL}>
              Guardar
            </Button>
          </div>
        </CardHeader>
                ...
                pricelists.map((pricelist, key) => {
                  return (
                    <TableRow key={key} className={classes.tableBodyRow} hover={true}>
                      <TableCell className={classes.tableCell}>{pricelist.articleId && pricelist.articleId}</TableCell>
                      <TableCell className={classes.tableCell}>
                        {pricelist.article.articleName && pricelist.article.articleName}
                      </TableCell>
                      <TableCell className={classes.tableCell}>
                        <input
                          id={pricelist.article.id}
                          onChange={handleChangeInput}
                          type="number"
                          className={classes.articleInput}
                          defaultValue={pricelist.percentage}
                        />{' '}
                        %
                      </TableCell>
                      <TableCell className={classes.tableCell}>
